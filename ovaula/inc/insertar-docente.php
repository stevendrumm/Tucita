<?php
	...
	if (isset($_POST['nombre']) & isset($_POST['apellido']) & isset($_POST['tipo']) & isset($_POST['documento']) & isset($_POST['email']) & isset($_POST['telefono'])) {
		...
		$sentencia="SELECT nombre FROM docente WHERE tipo_documento='$tipo' AND documento='$documento' AND cod_institucion='$institucion'";
		$num_rows=$generar->num_filas($sentencia);
		if ($num_rows==0) {
			$id=$tipo.$documento;
			$sentencia="INSERT INTO docente (id, nombre, apellido, tipo_documento, documento, email, telefono, cod_institucion) VALUES ('$id', '$nombre', '$apellido', '$tipo', '$documento', '$email', '$telefono', '$institucion')";
